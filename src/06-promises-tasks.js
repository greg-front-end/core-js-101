...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((resolve, rejcet) => {
    if (typeof isPositiveAnswer !== 'boolean') {
      rejcet(new Error('Wrong parameter is passed! Ask her again.'));
    }
    if (isPositiveAnswer) {
...
async function chainPromises(array, action) {
  const resArr = [];
  await array.map((promise) => promise.then((result) => resArr.push(result),
    () => {}));

  const funcRes = resArr.reduce((acc, el) => action(acc, el));
