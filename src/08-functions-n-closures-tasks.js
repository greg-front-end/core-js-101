...
function getPolynom(...rest) {
  return (x) => {
    let res = null;
    if (rest.length === 3) {
      res = rest[0] * x ** 2 + rest[1] * x + rest[2];
    }
    if (rest.length === 2) {
      res = rest[0] * x + rest[1];
    }
...
function memoize(func) {
  ...
  let prevResult;
  return () => {
    if (map.has(prevResult)) {
      return map.get(prevResult);
    }
    prevResult = func();
    map.set(prevResult);
    return map.get(prevResult);
...
function retry(func, attempts) {
  return () => {
    try {
      ...
      if (attempts) retry(func, attempts - 1)();
    }
    return func();
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const logData = JSON.stringify(args).slice(1, -1);
    logFunc(`${func.name}(${logData}) starts`);
    const res = func(...args);
    logFunc(`${func.name}(${logData}) ends`);
    return res;
  };
}
...
function getIdGeneratorFunction(startFrom) {
  let prevCallFunctionCount = (startFrom - 1);
  return () => {
    prevCallFunctionCount += 1;
    return prevCallFunctionCount;
  };
}
